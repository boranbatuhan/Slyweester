...
// Geçiş durumu için bir ref değişkeni tanımlayın
const showLoader = ref(false);

// Timer'ı saklamak için bir değişken tanımlayın
let timer;

// Router'ı alın
const router = useRouter();

// Sayfa değişikliklerini dinleyin ve geçiş durumunu güncelleyin
router.beforeEach((to, from, next) => {

  // Geçiş durumunu etkinleştirin
  if(from.name=='Login' || from.name===undefined ){
      showLoader.value = true;
  }

  // 1 saniye sonra geçiş durumunu devre dışı bırakın
  timer = setTimeout(() => {
...
// Komponent kaldırıldığında timer'ı temizleyin (memory leak önleme)
onBeforeUnmount(() => {
  clearTimeout(timer);
});
</script>
<template>
    <div v-if="showLoader" class="w-screen h-screen z-[99999] bg-sl-black overflow-hidden fixed top-0 left-0 flex items-center justify-center">       
            <svg version="1.0" xmlns="http://www.w3.org/2000/svg" 
            class="!text-sl-blue !fill-sl-blue animate-bounce"
 width="128" height="128" viewBox="0 0 500.000000 500.000000"
 preserveAspectRatio="xMidYMid meet">

<g transform="translate(0.000000,500.000000) scale(0.100000,-0.100000)"
class="!text-sl-blue !fill-sl-blue "
